...
import staticImage from "../assets/imgs/profilePhoto.png";
import {
  Calendar2Week,
  EmojiSmile,
  Image,
...
const CreatePost = () => {
  ...
  const name = "davide"; //useSelector((state)=>state.user.name)
  const surname = "alonzi"; //useSelector((state)=>state.user.surname)
  const profileImg = staticImage; //useSelector((state)=>state.user.image)
  ...
  return (
    <>
      <Container className="mb-5 px-3 py-2 rounded-2 bg-white">
        <Row>
          <Col xs={1}>
            <img
              style={{ height: "50px" }}
        ...
        >
          ...
          <Modal.Footer>
            {error == true && (
              <p className="text-danger me-5">Non puoi salvare un post vuoto</p>
            )}
            <Button
              variant="primary"
